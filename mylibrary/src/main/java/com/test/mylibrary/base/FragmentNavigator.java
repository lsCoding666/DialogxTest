 ...
 * destination using this Navigator must set a valid Fragment class name with <code>android:name
 * </code> or {@link Destination#setClassName(String)}.
 *
 * <p>The current Fragment from FragmentNavigator's perspective can be retrieved by calling {@link
 * FragmentManager#getPrimaryNavigationFragment()} with the FragmentManager passed to this
 * FragmentNavigator.
 *
 * <p>Note that the default implementation does Fragment transactions asynchronously, so the current
 * Fragment will not be available immediately (i.e., in callbacks to {@link
...
public class FragmentNavigator extends Navigator<FragmentNavigator.Destination> {
    ...
    public FragmentNavigator(
            @NonNull Context context, @NonNull FragmentManager manager, int containerId) {
        mContext = context;
     ...
     * {@inheritDoc}
     *
     * <p>This method must call {@link FragmentTransaction#setPrimaryNavigationFragment(Fragment)}
     * if the pop succeeded so that the newly visible Fragment can be retrieved with {@link
     * FragmentManager#getPrimaryNavigationFragment()}.
     *
     * <p>Note that the default implementation pops the Fragment asynchronously, so the newly
     * visible Fragment from the back stack is not instantly available after this call completes.
     */
    @Override
    ...
    /**
     * Instantiates the Fragment via the FragmentManager's {@link FragmentFactory}.
     *
     * <p>Note that this method is <strong>not</strong> responsible for calling {@link
     * Fragment#setArguments(Bundle)} on the returned Fragment instance.
     *
     ...
     * @deprecated Set a custom {@link FragmentFactory} via {@link
     *     FragmentManager#setFragmentFactory(FragmentFactory)} to control instantiation of
     *     Fragments.
     */
    ...
    public Fragment instantiateFragment(
        ...
        return fragmentManager
                .getFragmentFactory()
                .instantiate(context.getClassLoader(), className);
    }

    /**
     * {@inheritDoc}
     *
     * <p>This method should always call {@link
     * FragmentTransaction#setPrimaryNavigationFragment(Fragment)} so that the Fragment associated
     * with the new destination can be retrieved with {@link
     * FragmentManager#getPrimaryNavigationFragment()}.
     *
     * <p>Note that the default implementation commits the new Fragment asynchronously, so the new
    ...
    public NavDestination navigate(
        ...
        if (mFragmentManager.getFragments().size() > 0) {
            lastFragment =
                    mFragmentManager.getFragments().get(mFragmentManager.getFragments().size() - 1);
        ...
        final boolean isSingleTopReplacement =
                navOptions != null
                        && !initialNavigation
        ...
        if (initialNavigation) {
            ...
            findDestination(destination, lastFragment);
    ...
    void findDestination(Destination destNode, Fragment fragment) {
        if (fragment == null) {
            return;
        }
        //        if (mGraph == null) {
        ...
        //            return mGraph;
        //        }
        //        NavDestination destNode = mBackStack.isEmpty()
        //                ? mGraph
        //                : mBackStack.getLast().getDestination();
        NavGraph parentGraph = destNode.getParent();
        Iterator<NavDestination> iterator = destNode.getParent().iterator();
        while (iterator.hasNext()) {
            NavDestination destination = iterator.next();
            if (destNode.getClassName().equals(fragment.getClass().getName())) {
                Log.d("<<<", destination.getNavigatorName());
            } else {
                Log.d("<<<", destination.getNavigatorName());
            }
            Log.d("<<<", destination.getNavigatorName());
    ...
    public static class Destination extends NavDestination {
         ...
         *
         * @param navigatorProvider The {@link NavController} which this destination will be
         *     associated with.
         */
         ...
         * Fragment via {@link #setClassName(String)}.
         *
         * @param fragmentNavigator The {@link FragmentNavigator} which this destination will be
         *     associated with. Generally retrieved via a {@link NavController}'s {@link
            ...
            String className =
                    a.getString(
                            androidx.navigation.fragment.R.styleable
                                    .FragmentNavigator_android_name);
            if (className != null) {
    ...
    public static final class Extras implements Navigator.Extras {
        ...
        /**
         * Gets the map of shared elements associated with these Extras. The returned map is an
         * {@link Collections#unmodifiableMap(Map) unmodifiable} copy of the underlying map and
